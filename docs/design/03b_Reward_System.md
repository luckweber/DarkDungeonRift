# 03b — Sistema de Recompensa / Ecos · 🟢 P0 (design) / 🟡 P1 (código)
...
## 0. Nomenclatura — Eco (nome próprio do DDR)

| Termo | Uso | Equivalente em outros jogos |
|---|---|---|
| **Eco** (plural **Ecos**) | Upgrade escolhido **entre salas** que modifica a run | Boons (Hades), bênçãos (Death Must Die) |
| **Família** | Grupo temático de Ecos (Tempestade, Carniça…) | Deus/Olimpo (Hades), Deus (DMD) |
| **Pool** | Conjunto de Ecos disponíveis na run | — |

> 🔑 **Mesma lógica, nome próprio.** A mecânica é a dos roguelikes de escolha entre salas — só não chamamos de "Boon" no jogo. No código: `UDDREcoData`, `UDDREcoComponent`, tags `Eco.Family.*`, delegate `OnEcoAdded`.

---

## 1. A regra de ouro (convergência de 3 cadeiras)

> 🔑 **Um Eco bom transforma COMO você joga — não apenas um número.**

Kael (Combate), Mara (Roguelike) e Sofia (Produto) atacaram o mesmo ponto de ângulos diferentes na revisão: se os upgrades são só "+15% dano", o pilar (combo aéreo) vira a mesma animação 50 runs seguidas e a rejogabilidade morre. **Teste de aceite de todo Eco do MVP:**

```
"Isto muda como eu jogo o combo / a run?"
   SIM → Eco de verdade (maioria do pool)
   NÃO (só mexe num número) → Stat puro (minoria, "tempero", nunca o prato)
```

E o corolário, da cadeira de Combate: **Os Ecos DEVEM poder modular o pilar** — adicionar hit ao juggle, transformar o slam em AoE de fogo, dar reset de decay. Um roguelike de combo cujos upgrades não tocam o combo desperdiça o pilar.
...
|---|---|---|
| **Ecos de run (total)** | **15-20** | Com 6, o jogador vê tudo em 2 runs e o "só mais uma" morre. |
...
Cada família empurra um **estilo de jogo** diferente. 4 famílias × ~4-5 Ecos = ~18.

| Família | Fantasia | Exemplos de Eco (note como tocam o pilar) |
|---|---|---|
...
Sinergia = Eco A + Eco B produz um efeito **maior que a soma**. Defina ≥ 3-4 explícitas no MVP:
...
**Eixo de tensão (1, barato):** às vezes a escolha oferece um Eco **mais forte com um custo** — *"+30% dano, mas -15% vida máxima"*. Custo ~zero de implementação (é só mais um `GE` com modifier negativo) e transforma cada escolha numa decisão de verdade.
...
|---|---|
| Eco de stat ("+15% dano") | `GE_RunUpgrade_*` **Infinite** aplicado no ASC |
| Eco de mecânica ("dash causa dano") | concede/**troca** uma `GameplayAbility` |
| Eco condicional ("combo aéreo cura") | `GE` condicional + **GameplayTag** de gatilho (ex.: `State.Combat.InAir`) |
...
| Cap de juggle (quem é dono?) | **a ability** (`GA_AirAttack`) é dona do cap; Ecos ajustam um atributo `MaxJuggleHits` lido por ela — evita stacking quebrado de `GE` |
...
**Reescrita do meta-loop ([Core Loop §5](03_Core_Loop_Roguelike.md)):** gastar o recurso persistente deve, de preferência, **destravar Ecos/famílias novas no pool**, **desbloquear uma habilidade inicial**, ou **dar +1 slot de escolha** — coisas que mudam a *próxima run*, não só engordam um stat. Mantenha pequeno no MVP (3-5), mas que cada um **mude algo perceptível**.

---

## 8. Checklist

- [x] Planilha com **15-20 Ecos** em **3-4 famílias** (no papel, antes do M4) — ver **[40 — Catálogo](40_Eco_Pool_Catalog.md)** (18 Ecos)
- [ ] Maioria dOs Ecos **modifica mecânica** (passa no teste do §1), stat puro é minoria
- [x] Pelo menos **alguns Ecos modulam o pilar aéreo** (juggle/slam/launcher) — [40 §3](40_Eco_Pool_Catalog.md)
- [x] **≥ 3-4 sinergias** explícitas documentadas — [40 §4](40_Eco_Pool_Catalog.md)
...
→ **[40 — Catálogo de Ecos](40_Eco_Pool_Catalog.md)** (conteúdo MVP) · [44 — Data-Driven](../systems/44_Data_Driven.md) · [42 — Run Manager](../systems/42_Run_Room_Manager.md) · [Core Loop](03_Core_Loop_Roguelike.md) · [Roadmap M4](../17_Implementation_Roadmap.md) · [GAS (05)](../systems/05_GAS_Architecture.md).
